...
export const BooksResult = memo(() => {
  ...
  const booksItems = useAppSelector<ItemsType[]>(
    (state) => state.books.books.items,
  );
  const totalItems = useAppSelector<number | null>(
    (state) => state.books.books.totalItems,
  );
  const isLoading = useAppSelector<boolean>(
    (state) => state.books.books.isLoading,
  );
  const searchingBooksValue = useAppSelector<string>(
    (state) => state.books.books.query,
  );
        ...
        {totalItems === 0 || totalItems === null ? (
          ...
          booksItems.map((el) => {
            return (
              <Grid key={v4()} xs={12}>
                <Paper
                  className={isLoading ? s.grid_loading : s.grid}
                  elevation={2}
                >
                  <div className={s.img}>
                    <img
                  ...
                  >
                    <CardContent
                      className={isLoading ? s.cards_loading : s.cards}
                      sx={{ minWidth: 640, paddingTop: 0 }}
                    >
                      <Typography
                        sx={{ fontSize: 14 }}
                      ...
                      >
                        <a
                          href={el.volumeInfo.previewLink}
                          className={s.link}
                          target="_blank"
                        >
                      ...
                      <Typography sx={{ mb: 1.5 }} color="text.secondary">
                        <span>
                          {el.volumeInfo.authors &&
                            el.volumeInfo.authors.map((autor, i) =>
                              el.volumeInfo.authors.length - 1 === i
                                ? autor
                                : `${autor}, `,
                            )}
                        </span>
