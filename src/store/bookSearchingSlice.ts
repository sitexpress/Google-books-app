...
import {
  BooksCategoryType,
  BookSearchingSliceTypeExtended, BooksRelevanceType,
  ItemsType,
} from "../common/types/types";
import { searchBooks } from "../common/components/SearchingField";
...
export const fetchSearchingBookTC = createAsyncThunk(
  ...
  async (
    ...
    try {
      const { searchingBooksValue, pageNum, relevance, category, pagination } =
        payload;
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchSearchingBookTC.fulfilled, (state, action) => {
      if (action.payload) {
        ...
        if (
          action.payload.books.data.items &&
          action.payload.pagination === true
        ) {
          state.books.items.push(...action.payload.books.data.items);
        }
        if (
          action.payload.books.data.items &&
          action.payload.pagination === false
        ) {
